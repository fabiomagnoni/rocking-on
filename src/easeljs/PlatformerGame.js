...
(function (window) {
    ...
    function PlatformerGame(stage, contentManager, gameWidth, gameHeight) {
        ...
        document.onclick = function (e) {
            var fakeEvent = { keyCode: 87 };
            instance.handleKeyDown(fakeEvent);
        };
        window.addEventListener("deviceorientation", handleOrientation, true);
        function handleOrientation(event) {
            var x = event.beta;  // In degree in the range [-180,180]
            var y = event.gamma; // In degree in the range [-90,90]
            //document.getElementById("debug").textContent = x;
            if (x > 10) {
                var fakeEvent = { keyCode: 68 };
                instance.handleKeyDown(fakeEvent);
              //  document.getElementById("debug").textContent = "OK "+x;
            }
            if (x < -10) {
                var fakeEvent = { keyCode: 65 };
                instance.handleKeyDown(fakeEvent);
              //  document.getElementById("debug").textContent = "OKO "+x;
            }
        }
    ...
    PlatformerGame.prototype.UpdateScore = function () {
        if (scoreText === null) {
            ...
            scoreText = new Text("SCORE: 0", "bold 18px Arial", "yellow");
            scoreText.x = 10;
            scoreText.y = 34;
    ...
    PlatformerGame.prototype.SetOverlayCanvas = function () {
        ...
        // We center it
        var statusX = (this.gameWidth - oneOfThisOverlay.width)/6;
        var statusY = (this.gameHeight - oneOfThisOverlay.height)/6;
    ...
    PlatformerGame.prototype.LoadNextLevel = function () {
        ...
        var nextLevelUrl = window.location.href.replace('index.html', '') + "assets/levels/" + this.levelIndex + ".txt";
        try {
